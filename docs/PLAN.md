...
| 功能模块 | 选型方案 | 说明 |
|----------|----------|------|
| **核心框架** | **ESP-IDF v6.2+**（target: `esp32s31`） | esp32s31 最低完整支持线（v6.0 全系无 S31 工具链；v6.1 缺 esp_lvgl_adapter 的 DMA2D 支持，见 3.8 CI） |
...
```bash
# 1. 安装 ESP-IDF（master 或 release/v6.2；v6.0 无 esp32s31 支持）
...
| 5 | **模块/工具链新上市** | S31 2026-07 才量产；esp32s31 支持仅在 IDF **v6.2+**（v6.0 无）——固件锁定 release/v6.2（或 master）并随官方更新 |
...
| 资料 | 版本 | 用途 |
|------|------|------|
| [ESP-IDF Programming Guide (esp32s31)](https://docs.espressif.com/projects/esp-idf/en/latest/esp32s31/index.html) | v6.2+ | 开发指南 / API 参考 |
