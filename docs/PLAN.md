...
3. **RGB 时序（✅ 上板实测 2026-08-28，厂商官方源码值）**：**PCLK=46 MHz**、**H: HSW=60/HBP=120/HFP=106**、**V: VSW=4/VBP=20/VFP=20**，PCLK **上升沿**采样、VS/HS/DE 高有效——**~59.8 fps（60fps 达成）**，显示正常。⚠️ 46MHz 超出规格书标称上限（≤35.7MHz），实测可用、量产需高低温/批量验证（异常回退厂商默认 16MHz≈20.8fps）；初期 31MHz 在错误消隐下花屏/滚动，Adafruit 5793 参数（2/44/46、16/16/50）对本模组垂直滚动，均不适用
