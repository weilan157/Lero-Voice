...
| 组成 | 说明 |
|------|------|
| **编译容器** | `espressif/esp-idf-ci-action@v1` 是官方 **`espressif/idf` Docker 镜像**的封装（`esp_idf_version` 参数 = Docker Hub tag）。**`release-v6.1` 镜像 + 容器内升级 IDF 到 master 源码**（Docker Hub 仅有 6.1 及更早 tag——6.2 未发布；IDF master 含 `esp_async_color_convert`，DMA2D 芯片编译 esp_lvgl_adapter 必需：IDF<6.2 时其 include 未声明依赖的 `esp_async_fbcpy.h` 编译失败；6.2 发布后改回固定 tag）——**v6.0 / release-v6.0 全系不含 S31 工具链** |
...
| 问题 | 处理 |
|------|------|
| CI 报 `image not found` / tag 无效 | `esp_idf_version` 必须是 [Docker Hub 已发布的 tag](https://hub.docker.com/r/espressif/idf/tags)（如 `release-v6.1`） |
