...
static esp_err_t s_fail_and_idle(const char *detail)
{
...
esp_err_t ota_service_check_http(void)
{
    ...
    if (err != ESP_OK) {
        ...
        (void)s_fail_and_idle("meta fetch failed");
        (void)xSemaphoreGive(s_mutex);
        return ESP_OK;
    }
    ...
    if (err != ESP_OK) {
        ...
        (void)s_fail_and_idle("meta invalid");
        (void)xSemaphoreGive(s_mutex);
        return ESP_OK;
    }
    if (!is_newer) {
    ...
    if (!battery_ok) {
        (void)s_fail_and_idle("battery too low");
        (void)xSemaphoreGive(s_mutex);
        return ESP_OK;
    }
    ...
    if (err != ESP_OK) {
        (void)s_fail_and_idle("no OTA slot");
        (void)xSemaphoreGive(s_mutex);
        return ESP_OK;
    }
    ...
    if (err != ESP_OK) {
        ...
        (void)s_fail_and_idle("download/verify failed");
        (void)xSemaphoreGive(s_mutex);
        return ESP_OK;
    }
...
esp_err_t ota_service_apply_sd(void)
{
    ...
    if (err != ESP_OK) {
        (void)s_fail_and_idle("no SD update");
        (void)xSemaphoreGive(s_mutex);
        return ESP_OK;
    }
    ...
    if (err != ESP_OK) {
        ...
        (void)s_fail_and_idle("SD write/verify failed");
        (void)xSemaphoreGive(s_mutex);
        return ESP_OK;
    }
...
esp_err_t ota_service_confirm(void)
{
    ...
    (void)ota_state_battery_gate(&battery_ok);
    if (!battery_ok) {
        (void)s_fail_and_idle("battery too low at confirm");
        (void)xSemaphoreGive(s_mutex);
        return ESP_OK;
    }
    ...
    if (err != ESP_OK) {
        (void)s_fail_and_idle("set boot partition failed");
        (void)xSemaphoreGive(s_mutex);
        return err;
    }
