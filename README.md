...
| 模块 | 选型 |
|------|------|
| 核心框架 | ESP-IDF **v6.1+**（target: `esp32s31`；v6.0 版本线不含 S31 支持） |
...
| **ESP-IDF** | **v6.1+**（esp32s31 支持的最低版本线；**v6.0 无 S31 工具链**） | CMake/Ninja/工具链由 `install.sh` 自动安装 |
...
```bash
# 1. 安装 ESP-IDF（master 或 release/v6.1 分支；v6.0 无 esp32s31 支持）
